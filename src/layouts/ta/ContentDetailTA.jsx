...
import NilaiBimbinganTA from "./NilaiBimbinganTA";

const ContentDetailTA = ({ onClose, data, ket, userInfo }) => {
    ...
    const fetchSeminar = async () => {
      try {
        const response = await fetch(
          `https://inkptatif.xyz/seminar/seminar.php?nama=${encodeURIComponent(
            data.nama
          )}`
        );
  ...
  return (
    <div className="w-[85%] mx-auto p-8 border-2 border-secondary rounded-3xl flex relative bg-white h-screen overflow-y-auto">
      <div className="w-[40%] flex flex-col gap-y-8 border-secondary border-r pr-8">
        ...
        <div>
          <h1 className="-mb-1 text-2xl font-semibold line-clamp-1 text-secondary font-primary">
            <span className="font-sans font-bold">J</span>udul TA:
          </h1>
...
export default ContentDetailTA;
